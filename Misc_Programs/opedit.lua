...
local function delete()
  ...
  elseif cby < #buffer then
    ...
    setStatus("Print: [Ctrl+P]  Save: [Ctrl+S] Close: [Ctrl+W]")
  end
end
...
local controlKeyCombos = {[keyboard.keys.p]=true,[keyboard.keys.s]=true,
                          [keyboard.keys.w]=true,[keyboard.keys.c]=true,
...
local function onKeyDown(char, code)
  ...
  elseif keyboard.isControlDown() and controlKeyCombos[code] then
    ...
    if code == keyboard.keys.p then
      printer = component.openprinter
      if (printer ~= nil) then
        local lineCount, pageCount, pages = 0, 0, 0
        local lineLen = 30
        local document = ""
        local sentStr = {}
          ...
          if string.len(v) > lineLen then
            while string.len(v) > lineLen do
              ...
              if string.len(tmpStr) < lineLen then
                document = string.sub(tmpStr, 1, string.len(tmpStr) - 1)
              else
                --delimPos = strripos(tmpStr, " ")
                document = string.sub(tmpStr, 1, lineLen)
              end
              table.insert(sentStr, document)
              v = string.sub(v, lineLen + 1)
            end
          end
          if string.len(v) < lineLen then
            table.insert(sentStr, text.trim(v))
          end
        end
        for k, v in ipairs(sentStr) do
          if (pageCount < 9) then
            ...
            io.write("Printer tray full. Press [ENTER} when ready to continue")
            io.read()
            pageCount = 0
          end
        end
    ...
    elseif code == keyboard.keys.s and not readonly then
