...
local cpath = fs.path(shell.resolve(process.running(),nil))
local oldDirectory = os.getenv("PWD");
shell.setWorkingDirectory(cpath)
...
function compareVersions(v1,v2)
  ...
  function doUpdate(watdo)
    if (watdo == "update") then
      if autoUpdate == true then
        ...
        if doTehUpdate == "yes" then
          ...
          fs.remove(cpath .. "default.gss")
          fs.remove(cpath .. "cv.gss")
          fs.remove(cpath .. "gml.lua")
          fs.remove(cpath .. "gfxbuffer.lua")
          fs.remove(cpath .. "colorutils.lua")
          fs.remove(cpath .. "colorutils.lua")
          fs.remove(cpath .. "compviewer-version.txt")
          currFile = process.running()
          fs.remove(currFile)
          if not file_check(cpath .. currFile) then
    ...
    if not component.isAvailable("internet") then
      if not file_check(cpath .. "gml.lua") or not file_check(cpath .. "cv.gss") or not file_check(cpath .. "default.gss") or not file_check(cpath .. "gfxbuffer.lua") or not file_check(cpath .. "colorutils.lua") then
...
--local filename = shell.resolve(cpath..fname)
...
function loadInfoData(select)
  ...
  do
    local ccomp = component.getPrimary(select)
    if ccomp and optNameStart < 2 then
      for i in pairs(component.methods(ccomp.address)) do
        local cStr = tostring(ccomp[i])
        if cStr and string.len(cStr)>0 then
          if optNameLast < 1 then
            optNameLast = optNameLast + 1
          end
          local ch = ";"
          if not string.find(cStr, ";") then
            ch = "%-%-"
          end
          local ccStr = string.gsub(cStr,"(.*)"..ch..".*",i.." - %1")
          cStr = string.gsub(cStr,".*"..ch.."(.*)","%1")
          table.insert(tmpLine,optNameLast,ccStr)
          optNameLast = optNameLast + 1
          table.insert(tmpLine,optNameLast,cStr)
          optNameLast = optNameLast + 1
          lineCnt = lineCnt + 3
          stuffDone = true
        end
  ...
  if optNameStart < 2 and not stuffDone then
    table.insert(sentStr, "No information available for "..select)
  else
    table.insert(sentStr, tostring(select))
  end
  if lineCnt > 2 then
    local tStr = ""
    for k, v in pairs(tmpLine) do
      if k >= optNameStart and k <= optNameLast then
          v = text.trim(v)
          local dspace = 6
          if string.find(v,"function%(.*%)") then
            dspace = 3
          end
        if string.len(v) > lineLen then
          ...
          if string.len(v) < lineLen then
            table.insert(sentStr, spaces(dspace)..text.trim(v))
          end
        elseif string.sub(v, 1, string.len(select)) == select then
          table.insert(sentStr, v)
        else
          table.insert(sentStr, spaces(dspace)..v)
        end
      end
    end
  end
...
function newListBox()
  ...
  loadInfoData(select)
  term.clear()
  local labelText = "Functions explanation for "
  local infoLabel=infoGUI:addLabel(math.max(1,math.floor((infoGuiWidth-string.len(labelText))/2)), 1, infoFunctionsLabelWidth, labelText)
  local infoLabel2=infoGUI:addLabel(math.max(1,math.floor((infoGuiWidth-string.len(select))/2)), 2, infoFunctionsLabelWidth, select)
