...
local function printCompXY(menuSel)
	...
	if string.upper(string.char(key)) == "M" then
		...
		elseif string.lower(menuSel) == "redstone" then
			...
			printXY(10, 18, "setOutput(side: number, value: number): number"..spaces(5).."Sets the strength of the redstone signal to emit on the")
			printXY(20, 19, "specified side. Returns the new value.")
			printXY(20, 20, "This can be an arbitrarily large number for mods that support this. For vanilla interaction it is clamped to")
			printXY(20, 21, "the interval [0, 15].")
			printXY(10, 23, "getBundledInput(side: number, color: number): number"..spaces(5).."Like getInput, but for bundled input, reading")
			printXY(20, 24, "the value for the channel with the specified color.")
			printXY(10, 26, "getBundledOutput(side: number, color: number): number"..spaces(5).."Like getOutput, but for bundled output, getting the value")
			printXY(20, 27, "for the channel with the specified color.")
			printXY(10, 29, "setBundledOutput(side: number, color: number, value: number): number"..spaces(5).."Like setOutput, but for bundled output,")
			printXY(20, 30, "setting the value for the channel with the specified color.")
		else
